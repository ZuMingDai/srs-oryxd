...
func ServerRun(c *Config, callback func() int) int {
	...
	if c.LogToFile() {
		LoggerTrace.PrintIn("apply log file ", c.Log.File)
		if f, err := os.OpenFile(c.Log.File, os.O_WRONLY|os.O_APPEND|os.O_CREATE, 0644); err != nil {
			LoggerError.PrintIn("open log file", c.Log.File, "failed, err is", err)
			return -1
		} else {
			defer func() {
				err = f.Close()
	...
	LoggerTrace.PrintIn("apply workers", c.Workers)
